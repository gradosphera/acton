...
#[test]
fn test_get_executor_shared_across_threads() -> anyhow::Result<()> {
    const THREADS: usize = 4;
    const ITERATIONS: usize = 32;
    const STACK_B64: &str = "te6ccgEBAQEABQAABgAAAA==";
    ...
    let exec = Arc::new(GetExecutor::new(&params)?);
    ...
    for _ in 0..THREADS {
        ...
        handles.push(thread::spawn(move || -> anyhow::Result<()> {
            ...
            for _ in 0..ITERATIONS {
                let result = exec.run_get_method(STACK_B64, &params, None)?;
                assert!(matches!(result, GetMethodResult::Success(_)));
            }
...
#[test]
fn test_get_executor_parallel_instances() -> anyhow::Result<()> {
    const THREADS: usize = 4;
    const ITERATIONS: usize = 32;
    const STACK_B64: &str = "te6ccgEBAQEABQAABgAAAA==";
