...
**What you'll learn:**
- How to structure unit tests in Tolk
- How to use the expect API for assertions
...
All test files have the `_test.tolk` extension, for example, `wallet_test.tolk`, which allows you to quickly identify test files among others. 
To create a new test, write getter methods with names starting with `test_` or `test-`.

The simplest unit test looks like this:

```tolk title="unit_test.tolk"
...
<Callout type="warn">
Test functions must be getter methods (using `get fun`) and their names must start with `test_` or `test-`, otherwise the test runner will just ignore them.
</Callout>
...
Read the [complete guide to custom matchers](/docs/test-runner/writing-your-own-matchers/) for detailed examples and best practices.
</Callout>
...
**What we've covered:**
1. Setting up unit test files with the `_test.tolk` extension
...
- Test files are easily identifiable with the `_test.tolk` suffix
...
- Read the [complete guide to custom matchers](/acton/test-runner/writing-your-own-matchers) for advanced patterns
