...
**What you'll learn:**
- How to deploy contracts to testnet and mainnet
- Sending messages from wallet addresses
- Querying blockchain state
- Network selection and safety practices
- The difference between local testing and broadcasting

## Prerequisites

Before interacting with the real blockchain, you need to configure a wallet in your `Acton.toml`. Follow the [Setting up wallets](/acton/scripting/setup-wallets/) guide to configure your wallet with mnemonic and expected addresses.
...
fun main() {
    ...
    val result = net.send(wallet.address, msg, SEND_MODE_PAY_FEES_SEPARATELY);
    result.wait();
...
fun Counter.deploy(self, deployer: address, value: coins) {
    ...
    return net.send(deployer, msg, SEND_MODE_PAY_FEES_SEPARATELY);
}
...
- With `--broadcast`: Uses the real wallet configured in `Acton.toml`
...
fun main() {
    // ...
    val result = net.send(deployer.address, msg, SEND_MODE_PAY_FEES_SEPARATELY);
...
<Callout type="info">
Learn more about fork testing in the [Fork Testing](/acton/test-runner/fork-testing/) article.
</Callout>
...
- Check the [script command reference](/acton/script/) for all available options
- Learn more about [wallet configuration](/acton/scripting/setup-wallets/)
- Explore [verifying your deployed contracts](/acton/verify/)
