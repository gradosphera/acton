...
#[allow(dead_code)]
impl Project {
    pub(crate) fn litenode(&self) -> LiteNodeBuilder<'_> {
        LiteNodeBuilder::new(self)
    }
}

#[allow(dead_code)]
impl<'a> LiteNodeBuilder<'a> {
    ...
    pub(crate) fn start(self) -> LiteNodeHandle {
        ...
        let child = cmd.spawn().unwrap_or_else(|e| {
            panic!(
                "Failed to start `acton litenode start --port {}`: {}",
                self.port, e
            )
        });

        let mut handle = LiteNodeHandle {
            child: Some(child),
            port: self.port,
            ...
            client: Client::builder()
                ...
                .build()
                .expect("Failed to create HTTP client for lite node tests"),
        };
        ...
        match handle.wait_until_ready(self.ready_timeout) {
            ...
            Err(err) => {
                ...
                panic!(
                    "LiteNode failed to become ready on port {}: {}\n{}",
                    self.port, err, logs
                );
            }
        }
...
pub(crate) struct LiteNodeHandle {
    child: Option<Child>,
    port: u16,
    base_url: String,
    client: Client,
...
impl LiteNodeHandle {
    ...
    fn child_mut(&mut self) -> &mut Child {
        self.child
            .as_mut()
            .expect("LiteNode child process is not available")
    }
...
fn find_available_port() -> u16 {
    let listener = TcpListener::bind(("127.0.0.1", 0))
        .expect("Failed to reserve an ephemeral port for lite node tests");
    listener
        .local_addr()
