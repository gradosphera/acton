...
- Keep `tests/simple-extension.test.tolk`, `wrappers/SimpleExtension.gen.tolk`, `wrappers/WalletV5Contract.tolk`, and `scripts/` aligned with contract changes.
- When the SimpleExtension ABI changes, prefer regenerating `wrappers/SimpleExtension.gen.tolk` with `acton wrapper SimpleExtension` over hand-editing the wrapper file.
