...
#[test]
fn test_cpuprofile_via_config_exports_devtools_profile() {
    let project = ProjectBuilder::new("cpuprofile-config")
        .contract("simple", SIMPLE_CONTRACT)
        .test_file("profile", CPU_PROFILED_UNIT_TEST)
        .with_test_config(TestConfig {
            cpuprofile: Some("gas.cpuprofile".to_string()),
            profile_include_tests: Some(true),
            ..Default::default()
        })
        .build();
    ...
    output
        ...
        .assert_file_snapshot_matches(
            "gas.cpuprofile",
            "integration/snapshots/test_cpuprofile_via_config_exports_devtools_profile.cpuprofile",
        );
}

#[test]
fn test_profile_format_collapsed_via_config_exports_collapsed_stacks() {
    let project = ProjectBuilder::new("collapsed-profile-config")
        .contract("simple", SIMPLE_CONTRACT)
        .test_file("profile", CPU_PROFILED_UNIT_TEST)
        .with_test_config(TestConfig {
            cpuprofile: Some("gas.collapsed".to_string()),
            profile_format: Some("collapsed".to_string()),
            profile_include_tests: Some(true),
            ..Default::default()
        })
        .build();
    ...
    output
        ...
        .assert_file_snapshot_matches(
            "gas.collapsed",
            "integration/snapshots/test_profile_format_collapsed_via_config_exports_collapsed_stacks.collapsed",
        );
}
