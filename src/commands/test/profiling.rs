...
use acton_config::test::ProfileFormat;
...
pub(super) fn collect_profile(runner: &TestRunner) -> anyhow::Result<()> {
    ...
    if collect_snapshot_stats {
        ...
        if gas_per_opcode.is_empty()
            && trace_chain_stats.is_empty()
            && runner.config.cpuprofile.is_none()
        {
            return Ok(());
        }
        ...
        let baseline_snapshot = if let Some(baseline_path) = &runner.config.baseline_snapshot {
            match load_gas_snapshot(&runner.project_root, baseline_path) {
                ...
                Err(err) => {
                    ...
                    eprintln!(
                        "Warning: Failed to load baseline gas snapshot '{baseline_path}': {err}",
                    );
                    None
                }
            }
    ...
    if let Some(cpuprofile_filename) = &runner.config.cpuprofile {
        let executions = collect_profile_executions(runner);
        save_execution_profile(
            &executions,
            runner.config.profile_format,
            &runner.project_root,
            cpuprofile_filename,
        )?;
    }
...
fn save_execution_profile(
    ...
    match format {
        ProfileFormat::Cpuprofile => {
            let profile = build_devtools_cpu_profile(executions);
            save_devtools_cpu_profile(&profile, project_root, filename)?;
        }
        ProfileFormat::Collapsed => {
            let profile = build_collapsed_profile(executions);
            save_collapsed_profile(&profile, project_root, filename)?;
        }
    }
...
fn collect_execution_samples(execution: &ProfileExecutionInput) -> Vec<ProfileSample> {
    let vm_lines = parse_lines(&execution.vm_log);
    let trace = Trace::from_lines(vm_lines.clone(), Some(1_000_000));
    ...
    let Ok(mut replayer) = TolkReplayer::new(execution.source_map.as_ref(), &vm_lines) else {
        return Vec::new();
    };
...
fn save_devtools_cpu_profile(
    profile: &DevToolsCpuProfile,
    project_root: &Path,
    filename: &str,
) -> anyhow::Result<()> {
    ...
    let path = resolve_snapshot_path(project_root, filename);
    fs::write(path, json)?;
    println!("CPU profile saved to {filename}");
    Ok(())
}

fn save_collapsed_profile(
    profile: &str,
    project_root: &Path,
    filename: &str,
) -> anyhow::Result<()> {
...
impl ProfileFrameSpec {
    fn from_call_frame(
        ...
        let (url, line_number, column_number) = location
            .map(|loc| frame_location(replayer, loc))
