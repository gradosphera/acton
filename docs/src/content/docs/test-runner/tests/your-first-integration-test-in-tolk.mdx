...
First, as in the case with unit tests, let's create a file **counter_test.tolk** and declare a new test there:
...
Since we need to deploy a contract, we need a deployer account with some TON coins. We create one using `net.treasury("name")` — this gives us a test account that's automatically funded with coins. `net` is a global namespace that contains functions for sending messages, calling getter methods, and managing the test network.

When the message is created, we call `net.send()` to send it.

> Note that sending a message via `msg.send()` as in smart contracts will not work as you might expect. In fact, it will create an out action, but it will never be processed.
...
```
acton test counter_test.tolk --filter "test-should-increase-counter"
```
...
```tolk
 ...
 > counter_test.tolk (1 tests)
  ✗ test-should-increase-counter 21ms
    └─ Error: expect(actual).toEqual(expected)
        (
            100,
            0
        )
      └─ at counter_test.tolk:58:5
