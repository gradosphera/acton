...
| I need to stop a transaction chain between hops      | `net.sendIter()` and `TxCursor`                    | Transaction-chain control, not source stepping |
...
[IDE support](/docs/miscellaneous/ide-support/), but any DAP client can attach.
...
`tracing.save(...)` so exported artifacts stay readable.

## 4. Step-by-step Transaction Control with `TxCursor`

Acton also supports step-by-step transaction-chain control through
[`net.sendIter()`](/docs/standard_library/emulation/network/#netsenditer) and
[`TxCursor`](/docs/standard_library/emulation/network/#txcursor). This is
...
<Callout type="warn">
`net.sendIter()` is not source-level debugging. It is transaction-chain
