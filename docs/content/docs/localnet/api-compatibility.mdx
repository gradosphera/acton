...
| `GET`  | `/admin/status`        | No params                                                                                | Returns uptime, latest block seqno, and the active local or remote state source.                  |
| `POST` | `/admin/dump-state`    | JSON body: `{"path":"<SNAPSHOT_PATH>"}`                                                  | Writes a JSON state snapshot without stopping the server.                                         |
| `POST` | `/admin/load-state`    | JSON body: `{"path":"<SNAPSHOT_PATH>"}`                                                  | Replaces the current node state with a JSON state snapshot.                                       |
