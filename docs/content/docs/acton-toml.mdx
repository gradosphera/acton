...
`acton build --output-fift <DIR>` overrides this value for a single run.
...
`acton litenode start --port <PORT>`, `--fork-net <NETWORK>`, and `--fork-block-number <SEQNO>` override these values for a single run.
...
| `output`       | `object`   | Report output settings. Use `[lint.output.sarif]` to write SARIF report to file.                        |
...
CLI flag `acton check --sarif <PATH>` overrides `lint.output.sarif.path` for the current run.
