...
The manifest conforms to the [TOML 1.1 specification](https://toml.io/en/v1.1.0).
...
Pins project-level tooling versions. Acton checks this section before running
project commands and exits with an error when the installed CLI version does
not match the configured version.

| Field   | Type     | Required | Description                                                            |
|:--------|:---------|:---------|:-----------------------------------------------------------------------|
| `acton` | `string` | No       | Required Acton CLI version without a leading `v`, for example `1.0.0`. |
...
`acton up` remains available when the configured version does not match, so the
...
[Mutation Rules](/docs/testing/mutation-testing/rules).
...
The `fail-on-diff` is used together with baseline comparison mode (`acton test --baseline-snapshot <FILE>`).

## `[lint]`

Configures the Tolk linter (`acton check`).

Use `[lint]` for linter-wide settings and `[lint.rules]` for rule severities. For practical workflows and CI usage, see the [linting](/docs/lint) page.
...
See the [project lint policy configuration](/docs/lint#set-project-lint-policy) for more.
...
| Field      | Type     | Required                                | Description                                                                                                                                                                                                                                                                                     |
|:-----------|:---------|:----------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `api.v2`   | `string` | Yes for custom, optional for `localnet` | TON Center API v2 URL. For `localnet`, if omitted, Acton uses `http://localhost:<localnet.port>/api/v2`; if `[localnet].port` is not set, it falls back to port `5411`.                                                                                                                          |
| `api.v3`   | `string` | No                                      | TON Center API v3 URL. For `localnet`, if omitted, Acton uses `http://localhost:<localnet.port>/api/v3`; if `[localnet].port` is not set, it falls back to port `5411`.                                                                                                                          |
...
These defaults are written into `Acton.toml` by project scaffolding commands such as `acton new` and `acton init`. The exact path depends on the scaffold layout: standard templates use `contracts/`, `tests/`, and `wrappers/`, while `--app` scaffolds use `contracts/src/`, `contracts/tests/`, and `contracts/wrappers/`.
