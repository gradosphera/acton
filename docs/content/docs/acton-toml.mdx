...
Import groups are ordered as: `@stdlib`, `@acton`, `@<other>`, `../`, `./` (plain imports like `import "foo"` are treated as `./` group).
...
`acton litenode start --port <PORT>`, `--fork-net <NETWORK>`, `--fork-block-number <SEQNO>`, and `--accounts <NAME[,NAME...]>` override these values for a single run.
...
| `output-format` | `string`   | Default report format for `acton check`: `plain`, `json`, `sarif`, or `github`.                         |
