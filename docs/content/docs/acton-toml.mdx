...
| Field      | Type     | Required    | Description                                                                                                                                                     |
|:-----------|:---------|:------------|:----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `api.v2`   | `string` | Custom only | The URL for the TonCenter v2 API. For `localnet`, defaults to `http://localhost:<litenode.port>/api/v2` (or port `5411` if not set).                            |
| `api.v3`   | `string` | No          | The URL for the TonCenter v3 API.                                                                                                                               |
| `explorer` | `string` | No          | Base URL for transaction links in this network. Acton appends `/tx/<hash>` automatically (adds `/tx/` if missing). If omitted, links are derived from `api.v2`. |
...
You can then use this network in commands like `acton test --fork-net custom:my-custom-net`, `acton script --net custom:my-custom-net` or in scripts.

For `localnet`, `api.v2`/`api.v3` are optional: when omitted, Acton derives them from `[litenode].port` (fallback `3000`).
