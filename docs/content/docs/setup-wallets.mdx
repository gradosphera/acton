...
1.  **Local Wallets (`wallets.toml`)**: Project-specific wallets. Stored in project root for the current CLI invocation.
2.  **Global Wallets (`~/.config/acton/wallets/global.wallets.toml`)**: Shared across all your Acton projects.

Project root is selected by global flags. For exact resolution rules, see
...
Newly created wallets have zero balance. Before running `--broadcast` flows on testnet, fund the wallet with testnet TON.
...
Acton performs strict derived-vs-expected address verification when opening real wallets for blockchain operations (for example `acton script --broadcast`, `acton verify`, `acton library publish/topup`).
