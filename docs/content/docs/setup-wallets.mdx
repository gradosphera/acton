...
<Callout type="warn">
    Both `wallets.toml` and `global.wallets.toml` may contain sensitive information (mnemonics). Acton automatically adds them to your `.gitignore`, but you should always be careful not to commit them.
</Callout>
