...
2. **Compilation**: Each mutant compiles; mutants that fail to compile are reported separately and are not run through the test suite.
...
acton test --mutate --mutate-contract Wallet --mutation-disable-rules replace_plus_with_minus \
                                             --mutation-disable-rules replace_minus_with_plus
```
...
acton test --mutate --mutate-contract Wallet --mutation-diff worktree --mutation-levels critical \
                                             --mutation-disable-rules remove_logical_not
```
...
acton test --mutate --mutate-contract Wallet --mutation-diff worktree \
                                             --mutation-session-id wallet-worktree
```
