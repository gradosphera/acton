...
- [`vm.registerLibrary(...)`](/docs/standard_library/vm/vm/#vmregisterlibrary) registers a library in the current test blockchain so contracts can use it.
- [`net.fetchAndRegisterLibrary(...)`](/docs/standard_library/emulation/network/#netfetchandregisterlibrary) fetches a library by hash and registers it in the current test blockchain, returning `true` on success and `false` otherwise.
