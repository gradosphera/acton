...
| `-`           | `-v`, `--verbose` | Count   | Enable level-1 executor debug logs (CLI only) |

`--verbose` is currently binary in practice: default level `0`, and level `1`
when `-v` or `--verbose` is passed once. Higher user-selected levels are not
supported yet.
...
See [Fuzz Testing](/docs/test-runner/fuzz-testing) for annotation syntax,
