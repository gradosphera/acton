...
For TypeScript wrappers, you can also configure the default output directory in `Acton.toml`:
...
Then `acton wrapper counter --ts` writes `wrappers/Counter.ts` by default.
