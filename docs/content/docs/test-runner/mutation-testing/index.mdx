...
Use `--disable-rule` to exclude noisy or intentionally ignored rules:

```bash
acton test --mutate --mutate-contract wallet --disable-rule flip_plus --disable-rule flip_minus
```
...
```bash
acton test --mutate --mutate-contract wallet --mutation-diff worktree --mutation-levels critical --disable-rule remove_logical_not
```
...
`--disable-rule`, the IDs must still exist after those filters are applied.
...
Contract: Wallet
Source:   .jetton/contracts/jetton-wallet-contract.tolk
Files:    6
Mutants:  51
