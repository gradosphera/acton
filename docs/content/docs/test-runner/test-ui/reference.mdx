...
[`tracing.save(...)`](/docs/standard_library/emulation/tracing) to make trace
...
Logs tab shows `Executor Log` and `VM Log` grouped by transaction for the selected trace. If a transaction has no logs, it is skipped; if all VM logs are missing for the selected trace, UI shows a hint to rerun with `acton test --ui -v`.
