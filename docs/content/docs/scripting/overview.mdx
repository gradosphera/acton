...
Scripts can use all the functions that are used in tests, which means that within scripts it is possible to deploy a contract to a local blockchain, send messages, and call get methods. In the default emulation mode, state is not preserved between script runs. With `--net`, sent transactions affect the selected public network.
