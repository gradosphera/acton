...
<Callout>
  ...
  docker run --rm \
    -v "$PWD":/workspace \
    -w /workspace \
    ghcr.io/ton-blockchain/acton:{{ acton_version }} \
    build
  ```
...
To print the exact [JSON Schema](https://json-schema.org/) of the report to stdout, run:
...
Acton offers a GitHub action to set itself up. [`setup-acton`](https://github.com/ton-blockchain/setup-acton) downloads a release binary, restores and saves it with the GitHub Actions cache, adds `acton` to `PATH`, and exposes install details as outputs:
...
- name: Setup Acton
  ...
  with:
    ...
    # Supported values include 'latest', a release tag such as 'v{{ acton_version }}',
    # a bare semantic version such as '{{ acton_version }}', or 'trunk'.
...
If the repository also contains a frontend created with `acton new --app` or a custom one, create a separate frontend workflow with Node.js setup and validation steps:
...
For GitLab, run Acton from the published Docker image. The example pins the image to `{{ acton_version }}`; use `latest` only if the pipeline should always follow the newest release.

```yaml title=".gitlab-ci.yml"
image:
  name: ghcr.io/ton-blockchain/acton:{{ acton_version }}
  entrypoint: [""]
