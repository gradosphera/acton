...
# The setup-action produces following outputs: acton-path and acton-version.
- run: |
...
jobs:
  validate:
    ...
    steps:
      ...
      - name: Prettier # or other formatting tool such as Biome
        run: npm run fmt:check
```

## Set up GitLab CI

For GitLab, run Acton from the published Docker image. Replace `<ACTON_VERSION>` with the version used in the project, for example `1.0.0` or `latest`.
