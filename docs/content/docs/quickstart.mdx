...
<Steps>
    ...
    <Step title="Setup Wallet">
        ...
        This will generate a new wallet and save it to `wallets.toml` in your project root.
        By default, Acton stores mnemonic in your system's secure keyring/keychain (when available).

        <Callout type="warn">
            If secure keyring is unavailable, or you choose plain text storage (`--secure false`), `wallets.toml` will contain your private mnemonic. **Never commit it to git!** Acton automatically adds it to `.gitignore`.
        </Callout>
